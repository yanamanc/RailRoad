...
case choose
  ...
  when 4
  	...
  	route.stations.insert(-2, allstations.select{|station| station.name == station})
  ...
  when 5
    puts "Inter name of first and last station"
  	...
  	station = gets.chomp
  	route = routes.select{|route| route.stations[0] == first_station && route.stations[-1] == last_station}
  	route.stations.sub_station(allstations.select{|station| station.name == station})
    ...
    puts "1 - add to passanger train"
  	...
  	case choose
  	  when 1
  	  	puts "Inter nomber of train"
  	    nomber = gets.chomp.to_i
  	    trains.delete(stations.select{|station| station.name == station_del})
  	  when 2
  	  	puts "Inter nomber of train"
  	    nomber = gets.chomp.to_i
  	end
